...
const Appointment = () => {
  ...
  // Format date consistently with underscores
  ...
  const getAvailableSlot = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (i === 0) {
        // Round up to next 30-min slot if today
        let minutes = currentDate.getMinutes();
        let nextMinutes = minutes <= 30 ? 30 : 0;
        let nextHour =
          minutes > 30 ? currentDate.getHours() + 1 : currentDate.getHours();
        if (nextHour < 10) nextHour = 10; // earliest start
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      if (!selectedSlot) {
        toast.warn("Invalid slot selection");
        return;
      }

      const slotDate = formatSlotDate(selectedSlot.datetime);
